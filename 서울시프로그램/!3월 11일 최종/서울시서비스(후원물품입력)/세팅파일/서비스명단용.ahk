...
button1:
    ...
    guicontrol,, 진행률, 0
    ifexist, %a_workingdir%\세팅파일\서비스명단db.ini                              ;
        iniread, 명단목록, %a_workingdir%\세팅파일\서비스명단db.ini, 명단목록       ;중요한두줄이당
    gui, submit, nohide
    if(입력값="") ;입력칸이 빈칸이면
        msgbox 명단명을 적으세요.
    else
    {
		...
		sort(clipboard,입력값) ;실패명단저장 성공한거html로 저장하기
        IfNotEqual, 카운트, 0
        {
            iniread,x, %a_workingdir%\세팅파일\서비스명단db.ini, 명단목록        ;gui변수 바꿔주기 일단 그 목록값 x에 읽어내서 첨인지 아닌지 알아내야함(목록 gui 다시 만들기)
            if(x="")
            {
                iniwrite, %입력값%, %a_workingdir%\세팅파일\서비스명단db.ini, 명단목록
                Y:=입력값
            }
            else
            {
                Y=%x%|%입력값%
                inidelete, %a_workingdir%\세팅파일\서비스명단db.ini, 명단목록
                iniwrite, %Y%, %a_workingdir%\세팅파일\서비스명단db.ini, 명단목록
            }
...
button2:
    ...
    else
    {
        ...
        inidelete, %a_workingdir%\세팅파일\서비스명단db.ini, 명단목록
        iniwrite, %A%, %a_workingdir%\세팅파일\서비스명단db.ini, 명단목록
        inidelete, %a_workingdir%\세팅파일\서비스명단db.ini, %getITEM% ;해당section지우기
    }
    return
...
{   
    ...
    IfNotEqual, count, 0
        iniwrite, %http뽑기%, %a_workingdir%\세팅파일\서비스명단db.ini, %c%
